...
class BettingService:
    ...
    async def refresh_events(self, *, priority: bool = False) -> tuple[int, list[dict], list[str]]:
        ...
        Provider IDs are stored separately from Oddium event IDs so an existing
    ...
    async def refresh_odds(self, *, priority: bool = False, discover: bool = True) -> tuple[int, list[str]]:
        """Refresh real Bet365 prices from 5Dollar Pro.
            ...
            try:
                ...
                for match in rows:
                    ...
                    if quote:
                        ...
                        # Never invent or estimate a price when 5Dollar/Bet365 has none.
                        # The match remains visible but betting is unavailable until the
                        # authoritative 5Dollar market appears.
                        await self.db.execute("UPDATE matches SET odds_available=0 WHERE event_id=?", (match["event_id"],))
                        continue
    ...
    async def _reconcile_open_tickets(self) -> list[dict]:
        """Resolve pending tickets exclusively from 5Dollar final results.

        Priority: exact 5Dollar fixture id, then the native 5Dollar finished league feed.
        Unknown/unconfirmed matches remain pending; no other football provider is consulted.
        """
        now = datetime.now(timezone.utc)
        ...
        async def finish(row, parsed):
            ...
            await self.db.execute(
                ...
                (hs_i, aws_i, parsed.get("source") or "5Dollar fixture detail", fid, utcnow_iso(), event_id),
            )
        ...
        def match_candidate(row, shell):
            ...
            # Provider status payloads do not always use the literal word "finished".
        ...
        for row in rows:
            ...
            # 5Dollar is the only football truth source. No cross-provider
            # result matching is allowed anymore. If 5Dollar has not confirmed the
            # result, the ticket stays pending rather than importing another feed.
    ...
    async def _promote_scheduled_kickoffs(self) -> list[dict]:
        """Never infer LIVE status solely from the scheduled kickoff.
    ...
    def _sort_live_rows(self, raws: list[dict], sport_key: str) -> list[dict]:
        """Process weaker states first and authoritative 5Dollar live states last.

        Sorting remains useful when the native feed contains repeated observations
        for the same fixture during one refresh cycle.
        """
        state_rank = {
    ...
    async def refresh_scores_and_settle(self, *, force_live_panel: bool = False) -> tuple[int, list[dict], list[str]]:
        ...
        - Ticket settlement uses confirmed 5Dollar fixture data.
        """
        active = await self.active_competitions()
        score_updates = 0
        settlements: list[dict] = []
        ...
        # because repeated 5Dollar rows can enqueue the same
        ...
        # One-time compatibility sanitation: old releases mixed providers in the same
        ...
        # Le moteur Live doit fonctionner dès que le panneau principal
        ...
        # Le moteur Live est un service autonome. Il tourne même sans panneau
        ...
        for key in live_keys:
            ...
            # Hot polling only for competitions that actually have a nearby/live
            ...
            if live_panel_enabled and not (has_local_candidate or has_pending_bet):
                ...
                if discovery_due:
                    ...
                    need_scores = True
            # On exécute toujours le passage logique pour toutes les compétitions actives.
            ...
            try:
                ...
                # 5Dollar-only core: the football state is never co-authored
                # by another provider. Global live board + exact fixture detail are
                # the only live inputs.
                import asyncio

                async def _safe_live_call(label, coro, timeout=10.0):
                    try:
                        return await asyncio.wait_for(coro, timeout=timeout)
                    except Exception as exc:
                        try:
                            import logging
                            logging.getLogger("oddium").warning("%s indisponible pour %s: %s", label, COMPETITIONS.get(key, {}).get("name", key), exc)
                        except Exception:
                            pass
                        return []

                raws = []
                score_source = "5DollarFootballAPI"
                if SETTINGS.five_dollar_api_key:
                    raws = list(await _safe_live_call("5Dollar", self.odds_api.fetch_five_dollar_live(key, force=force_api or discovery_due), 10.0) or [])
                    ...
                    try:
                        ...
                        direct_rows = []
                    raws.extend(direct_rows or [])
                if not raws:
                    score_source = "5DollarFootballAPI • aucune donnée"

                try:
                    import logging
                    logging.getLogger("oddium").info(
                        "Oddium Live %s: source=%s | rows=%s",
                ...
                for raw in raws or []:
                    ...
                    if old is None:
                        ...
                        await self.db.execute(
                            """INSERT INTO matches(event_id,sport_key,competition_name,home_team,away_team,home_team_id,away_team_id,commence_time,
                                   odds_available,completed,cancelled,home_score,away_score,match_status,live_phase,live_clock,live_detail,live_source,five_dollar_fixture_id,last_score_update,first_seen_at,last_seen_at)
                               VALUES(?,?,?,?,?,?,?,?,0,0,0,?,?,?,?,?,?,?,?,?,?,?)""",
                            (provider_event_id, key, event.get("competition_name") or COMPETITIONS.get(key, {}).get("name", key), event["home_team"], event["away_team"],
                             ...
                             event.get("live_clock"), event.get("status_detail"), event.get("source"), event.get("five_dollar_fixture_id"),
                    ...
                    if new_phase == "pending" and old_phase in {"kickoff_wait","live","first_half","halftime","second_half","extra_time","penalties","suspended"}:
                        new_phase = old_phase
                        status = old_phase
                    # Generic IN_PLAY must not erase a more precise known
                    ...
                    # Preserve the last confirmed 5Dollar clock when a later snapshot omits it.
                    ...
                    if new_phase in stopped_phases:
                        ...
                        # Do not let an out-of-order 5Dollar snapshot move the clock backwards.
                    ...
                    await self.db.execute(
                        ...
                        (hs_i, aws_i, 1 if changed else 0, signal_ts, status, new_phase, new_clock or None, new_detail or None,
                         ...
                         event.get("five_dollar_fixture_id"), target_event_id),
                    )
                    if changed:
                        score_updates += 1
    ...
    def _live_row_rank(row):
        ...
        try:
            ...
            minute = 0
        # Prefer the most advanced coherent 5Dollar state when duplicate rows exist.
    ...
    def _dedupe_match_rows(self, rows, limit: int = 25):
        ...
        legal suffixes, so exact keys can still produce duplicates.
        """
    ...
    async def api_status(self) -> dict:
        ...
        for key in await self.active_competitions():
            diagnostics.append({
                ...
                "odds_source": str(await self.db.get_setting(f"diag_odds_source_{key}") or "5DollarFootballAPI"),
            })
        try:
